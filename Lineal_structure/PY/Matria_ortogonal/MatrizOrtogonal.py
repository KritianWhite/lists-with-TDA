...
class MatrizOrtogonal:
    ...
    def ubicarCoordenada(self, fila, columna, dato_nuevo):
        tmp = None
        for i in range(1, 26):
            tmp = self.filas.getEncabezado(i).getAcceso()
            while tmp != None:
                if tmp.x == fila and tmp.y == columna:
                    tmp.caracter = dato_nuevo
        #     try:
        #         tmp = self.filas.getEncabezado(fila).getAcceso()
        #         while tmp != None:
        #             print(tmp.x)
        #             print(tmp.y)
        #             inicio = None
        #             tmp2 = None
        #             inicio = self.columnas.getEncabezado(columna)
        #             tmp2 = inicio.getAcceso()
        #             while tmp2 != None:
        #                 if tmp.x == fila and tmp2.y == columna:
        #                     tmp.caracter = dato_nuevo
        #                     return tmp2
        #                 tmp2 = tmp2.getAbajo()
        #             tmp = tmp.derecha()
